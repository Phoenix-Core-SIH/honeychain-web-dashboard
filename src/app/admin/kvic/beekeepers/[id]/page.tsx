...
import { ArrowLeft, Warning, Hexagon, User } from '@phosphor-icons/react';
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <div className="flex items-center mb-8">
        <button onClick={() => window.history.back()} className="p-2 mr-4 hover:bg-slate-200 rounded-full transition text-muted-fg">
      ...
      {data && (
        <div className="space-y-8">
          <div className="bg-card-bg p-6 rounded-2xl border border-card-border shadow-sm flex items-center">
            ...
            <div>
              ...
              <p className="text-muted-fg">{data.hive_count} Registered Hives</p>
            </div>
          </div>
          
          <div>
            <h3 className="text-lg font-bold text-foreground mb-4">Batch History</h3>
            <div className="bg-card-bg rounded-2xl border border-card-border overflow-hidden shadow-sm">
              <table className="w-full text-left">
                <thead className="bg-background border-b border-card-border text-muted-fg text-sm">
                  <tr>
                ...
                <tbody className="divide-y divide-slate-100">
                  {data.batches.length > 0 ? data.batches.map((batch: any, i: number) => (
                    <tr key={i} className="hover:bg-background transition">
                      <td className="px-6 py-4 font-mono text-xs text-muted-fg">
                        <Link href={`/admin/batches/${batch.batch_id}`} className="text-amber-600 hover:underline">
                          {batch.batch_id.split('-')[0]}
                        </Link>
                      </td>
                      <td className="px-6 py-4 text-sm text-foreground">{new Date(batch.harvest_date).toLocaleDateString()}</td>
                      <td className="px-6 py-4 text-sm text-foreground">{batch.honey_type}</td>
                      <td className="px-6 py-4 text-sm font-semibold text-foreground">{batch.quantity_kg} kg</td>
                    </tr>
                  )) : (
                    <tr>
                      <td colSpan={4} className="px-6 py-8 text-center text-muted-fg">No batch history found.</td>
                    </tr>
                  )}
                </tbody>
              </table>
