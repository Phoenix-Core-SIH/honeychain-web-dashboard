...
import { ArrowLeft, Warning, MapPin, CheckCircle, Package } from '@phosphor-icons/react';
    ...
    <div className="p-8 max-w-5xl mx-auto">
      ...
      {data && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="bg-card-bg p-8 rounded-2xl border border-card-border shadow-sm flex flex-col items-center justify-center">
             <Package className="w-12 h-12 text-primary mb-4" />
             <p className="text-4xl font-bold text-foreground">{data.batches_this_period}</p>
             <p className="text-muted-fg font-medium uppercase text-sm mt-2 tracking-wider">Batches Produced</p>
          </div>
          
          <div className="bg-card-bg p-8 rounded-2xl border border-card-border shadow-sm flex flex-col items-center justify-center">
             {data.recent_disease_alerts > 0 ? (
               <Warning className="w-12 h-12 text-red-500 mb-4" />
             ) : (
               <CheckCircle className="w-12 h-12 text-green-500 mb-4" />
             )}
             <p className={`text-4xl font-bold ${data.recent_disease_alerts > 0 ? 'text-red-600' : 'text-foreground'}`}>{data.recent_disease_alerts}</p>
             <p className="text-muted-fg font-medium uppercase text-sm mt-2 tracking-wider">Recent Disease Alerts</p>
          </div>
        </div>
      )}
    </div>
