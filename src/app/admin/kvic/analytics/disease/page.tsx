...
import { ChartLineUp, MapPin, Warning, ArrowLeft } from '@phosphor-icons/react';
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="flex items-center mb-8">
        <Link href="/admin/kvic/clusters" className="p-2 mr-4 hover:bg-slate-200 rounded-full transition text-muted-fg">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-foreground flex items-center">
            <ChartLineUp className="w-6 h-6 mr-2 text-primary" />
            Disease Alerts Analytics
          </h1>
          <p className="text-muted-fg mt-1">Geospatial overview of hive health issues</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-primary rounded-2xl border border-slate-800 p-6 flex flex-col items-center justify-center min-h-[400px] shadow-sm relative overflow-hidden">
          ...
          <ChartLineUp className="w-16 h-16 text-primary mb-4 opacity-50" />
          <p className="text-slate-300 font-medium">Map Visualization (Placeholder)</p>
          <p className="text-muted-fg text-sm mt-2 max-w-sm text-center">
        ...
        <div className="bg-card-bg rounded-2xl border border-card-border shadow-sm overflow-hidden">
           <div className="px-6 py-4 border-b border-card-border bg-background">
             <h2 className="font-bold text-foreground">Alerts by Region</h2>
           </div>
           <div className="divide-y divide-slate-100">
             {data.length > 0 ? data.map((item, i) => (
               <div key={i} className="p-6 flex items-center justify-between hover:bg-background transition">
                 <div>
                 ...
                 <div className="text-right">
                   ...
                   <p className="text-xs font-medium text-muted-fg uppercase tracking-wider">Cases</p>
                 </div>
               </div>
             )) : (
