...
import { ArrowLeft, PaperPlaneRight, QrCode, MapPin, Clock, Hexagon, FileText } from '@phosphor-icons/react';
    ...
    <div className="p-8 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">Batch {data.batch_id.split('-')[0]}</h1>
            <p className="text-muted-fg mt-1 flex items-center">
        ...
        <div className="flex gap-3">
          {(role === 'ADMIN' || role === 'PROCESSOR_OPS' || role === 'BEEKEEPER_OPS') && (
            <button 
              ...
              className="flex items-center px-4 py-2.5 bg-card-bg border border-slate-300 text-foreground font-medium rounded-lg hover:bg-background transition shadow-sm"
            >
          ...
          <button 
            ...
            className="flex items-center px-4 py-2.5 bg-primary text-white font-medium rounded-lg hover:bg-primary-hover transition shadow-sm"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-card-bg rounded-2xl border border-card-border p-6 shadow-sm">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-foreground font-medium">{data.honey_type}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-foreground font-medium">{data.quantity_kg} kg</p>
              </div>
              <div>
                <p className="text-xs font-semibold text-slate-400 uppercase">Beekeeper</p>
                <p className="text-foreground font-medium">{data.beekeeper_name}</p>
              </div>
              <div>
                <p className="text-xs font-semibold text-slate-400 uppercase">Harvest Date</p>
                <p className="text-foreground font-medium">{data.harvest_date}</p>
              </div>
            </div>
          </div>
          
          <div className="bg-background rounded-2xl border border-amber-100 p-6">
            <h3 className="font-bold text-amber-900 mb-2 flex items-center">
              <Hexagon className="w-5 h-5 text-primary mr-2" />
              Current Custodian
            </h3>
        ...
        <div className="lg:col-span-2">
          <div className="bg-card-bg rounded-2xl border border-card-border p-6 shadow-sm min-h-[500px]">
            <h3 className="font-bold text-foreground mb-6 flex items-center border-b border-card-border pb-4">
              <MapPin className="w-5 h-5 text-primary mr-2" />
              On-Chain Custody History
            </h3>

            <div className="relative pl-8 border-l-2 border-card-border space-y-10 py-4">
              {/* Genesis Node */}
              <div className="relative">
                <div className="absolute w-4 h-4 bg-slate-300 rounded-full -left-[2.25rem] top-1 border-4 border-white"></div>
                <h4 className="font-bold text-foreground">Harvested at Hive ({data.hive_id})</h4>
                <p className="text-sm text-muted-fg mt-1 flex items-center">
              ...
              {data.custody_chain.map((step) => (
                <div key={step.step} className="relative">
                  <div className="absolute w-4 h-4 bg-background0 rounded-full -left-[2.25rem] top-1 border-4 border-white"></div>
                  <h4 className="font-bold text-foreground">Transferred to {step.stage}</h4>
                  <div className="mt-2 space-y-1">
                    <p className="text-sm text-muted-fg flex items-center">
      ...
      {showTransfer && (
        <div className="fixed inset-0 bg-primary/50 flex items-center justify-center p-4 z-50">
          <div className="bg-card-bg rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="p-6 border-b border-card-border">
              <h2 className="text-xl font-bold text-foreground">Transfer Custody</h2>
            </div>
            <form onSubmit={handleTransfer} className="p-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">To Address *</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Transfer Stage *</label>
                  <select value={transferType} onChange={(e) => setTransferType(Number(e.target.value))} className="w-full px-4 py-2 rounded-lg border border-slate-300 focus:ring-2 focus:ring-amber-500 outline-none bg-card-bg">
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Location JSON (Optional)</label>
              ...
              <div className="mt-8 flex justify-end gap-3">
                <button type="button" onClick={() => setShowTransfer(false)} className="px-4 py-2 text-muted-fg font-medium hover:bg-slate-100 rounded-lg">Cancel</button>
                <button type="submit" disabled={actionLoading} className="px-6 py-2 bg-primary text-white font-medium rounded-lg hover:bg-primary-hover disabled:opacity-70 flex items-center">
      ...
      {showQR && (
        <div className="fixed inset-0 bg-primary/50 flex items-center justify-center p-4 z-50">
          <div className="bg-card-bg rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="p-6 border-b border-card-border bg-background">
            ...
            <form onSubmit={handleActivateQR} className="p-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Jar Serial / QR ID *</label>
              ...
              <div className="mt-8 flex justify-end gap-3">
                <button type="button" onClick={() => setShowQR(false)} className="px-4 py-2 text-muted-fg font-medium hover:bg-slate-100 rounded-lg">Cancel</button>
                <button type="submit" disabled={actionLoading} className="px-6 py-2 bg-background0 text-white font-medium rounded-lg hover:bg-primary-hover disabled:opacity-70 flex items-center shadow-sm">
